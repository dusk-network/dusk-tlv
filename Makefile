...
publishdoc: ## Generate and publish git pages docs
	@cargo +nightly doc && \
		echo "<meta http-equiv=refresh content=0;url=`echo 'https://dusk-network.github.io/dusk-tlv/dusk_tlv/index.html' | cut -d '/' -f 2`/index.html>" > target/doc/index.html && \
		curl -o 'target/doc/badge.svg' 'https://img.shields.io/badge/docs-latest-blue?logo=rust' && \
		curl -o 'target/doc/repo-badge.svg' 'https://img.shields.io/badge/github-dusk--tlv-brightgreen?logo=github' && \
		ghp-import -n target/doc && \
		git push -f https://github.com/dusk-network/dusk-tlv gh-pages
